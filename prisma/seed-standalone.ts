...
async function main() {
  console.log('🌱 Starting MongoDB standalone compatible database seeding...')

  try {
    let createdCategories = 0
    let createdProducts = 0

    // Create categories (standalone MongoDB compatible)
    ...
    for (const categoryData of categories) {
      try {
        ...
        console.log(`⚠️ Category ${categoryData.name} creation failed:`, error instanceof Error ? error.message : String(error))
      }
    }

    // Create products (standalone MongoDB compatible)
      ...
      try {
        ...
        if (!existingProduct) {
          ...
          if (category) {
            ...
            console.log(`⚠️ Category ${productData.categoryName} not found for product ${productData.name}`)
          }
        } else {
          console.log(`ℹ️ Product already exists: ${productData.name}`)
        }
      } catch (error) {
        console.log(`⚠️ Product ${productData.name} creation failed:`, error instanceof Error ? error.message : String(error))
      }
    }
    ...
    console.log(`   Total Categories: ${categoryCount} (${createdCategories} newly created)`)
    console.log(`   Total Products: ${productCount} (${createdProducts} newly created)`)
