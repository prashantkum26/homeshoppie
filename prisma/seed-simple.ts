...
async function main() {
  console.log('🌱 Starting simple MongoDB seeding (no transactions)...')

  try {
    // Step 1: Create categories using upsert
    ...
    for (const productData of products) {
      try {
        ...
        console.log(`✅ Product processed: ${productData.name}`)
        productsCreated++
      } catch (error) {
    ...
    console.log(`   Products processed in this run: ${productsCreated}`)
